//! `mineradio://` custom-protocol handler.
//!
//! Frontend `api.ts` rewrites media URLs onto the `mineradio` scheme so that
//! audio/image bytes that must be fetched (or decrypted) land on the Rust side
//! instead of a separate sidecar process. The three routes below mirror the
//! old sidecar proxy routes:
//!
//! * `/audio-proxy?url=…` — plain audio passthrough (full-body fetch).
//! * `/image-proxy?url=…` — cover passthrough with a browser user-agent + referer.
//! * `/providers/soda/audio-proxy?url=…&playAuth=…` — soda audio fetch + decrypt.
...
pub fn handle_media_request<R: tauri::Runtime>(
    ...
    tauri::async_runtime::spawn(async move {
        let response = route_media(&path, &query).await;
